...
public class DBManager {
    ...
    public List<Area> queryArea(String parentId) {
        ...
        Cursor c = db.rawQuery("SELECT * FROM v_area WHERE parent_id = ?", new String[]{parentId});
        while (c.moveToNext()) {
            Area area = new Area();
            area.setCode(c.getString(c.getColumnIndex("code")));
            area.setParentId(c.getString(c.getColumnIndex("parent_id")));
            area.setArea(c.getString(c.getColumnIndex("area")));
            area.setPinyin(c.getString(c.getColumnIndex("pinyin")));
