...
import com.rthc.freshair.willianchartview.model.LineSet;
import com.rthc.freshair.willianchartview.model.Point;
	...
	public ArrayList<ArrayList<Region>> defineRegions(ArrayList<ChartSet> data){
		...
		for(ChartSet set : data){
			...
			for(ChartEntry e : set.getEntries()){
				...
				float sss = mClickableRadius;
				x = e.getX();
				y = e.getY();
				regionSet.add(new Region((int)(x - sss),
											(int)(y - sss),
												(int)(x + sss),
													(int)(y + sss)));
			}
